...
export async function POST(
  ...
  if (!session?.externalId) {
    return NextResponse.json(
      { error: "Sign in to contribute to a mandate.", code: "SIGN_IN" },
      { status: 401 }
    );
  }
  ...
  try {
    const mandate = await contributeToMandateApi(session.externalId, id, owls);
    return NextResponse.json({ mandate });
  } catch (err) {
    if (err instanceof AccountApiError) {
      const error =
        err.code === "INSUFFICIENT_OWLS"
          ? "Your balance cannot cover that contribution. You can buy owls " +
    ...
    console.error("[minerval] mandate contribution failed:", err);
    return NextResponse.json(
