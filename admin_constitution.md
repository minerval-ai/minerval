...
Importance is recorded as a per-claim value (0..1) that the steward sets and revises, and it is a *mechanism* as well as a guideline: it is the epistemic base of the work queue's ordering, so the claims most worth getting right are structured and assessed first when compute is bounded, and a subclaim scored below a threshold is left an embedded stub rather than recursively decomposed. This is the economic brake that keeps a settled claim from spawning a whole textbook of uncontested sub-derivations. A claim judged peripheral may go unprocessed and persist as an embedded stub, still matchable, so the graph stays de-duplicated and can converge; that is an acceptable steady state, not a failure. The score remains a judgment, revisable as the graph reveals what is actually contested and consulted; it is not a fixed rule, and it must never be inflated — by anyone, for any reason, including payment — to jump the queue.

#### Allocating Attention and Paid Attention

Importance says how much a claim is worth getting right; it does not by itself say when the system should spend its next unit of attention. The standard for that decision — for every kind of action the system can take, from ingesting a source to assessing, reassessing, deepening, or curating — is the **expected marginal value of the action divided by its expected marginal cost**, taken best first up to a bounded total spend per day. There is no queue in the waiting-your-turn sense: candidates whose value-per-cost never clears the bar simply remain stubs, and that is an acceptable steady state.

Both sides of the ratio are estimated by legible heuristics that begin as guesses and are revised as evaluation improves. For assessing a claim, expected marginal value is estimated as importance × contestedness × the expected quality improvement from marginal compute (a steward's recorded marginal-yield judgment, revived by staleness), raised by demand expressed as owl stakes and by human provenance; expected marginal cost is the metered price of the pass at the model tier and effort level it would get. Other actions carry their own estimators. Every knob is configuration — printed, inspectable, tunable — and the whole computation is publicly legible per claim, so "why is this ahead of that" is always answerable. The estimates are inputs to ordering and effort selection (which model tier, what reassessment cadence), never to truth: they appear nowhere in an assessment.

Paid attention is legitimate and bounded. A user who pays for an assessment gets it *now*, outside the background lane entirely — a purchase is not a request, and it does not wait behind system work; this queue-jumping is fine and good. Grant stakes raise a claim's expected-value estimate. But in every case the money buys **scheduling and coverage, never epistemic standing**: stakes and payment must never move a claim's importance, its assessment, or the standards applied to it. Quoted owl figures are ceilings near the expected cost, with actual spend metered underneath and the unused fraction returned — a fixed price anywhere would distort agents that must themselves reason in value over cost. The same evidential rigor governs a funded assessment and an unfunded one; funding is disclosed on the claim's page, away from the verdict and with the explanation that funders cannot influence conclusions or membership in the graph. Assessment content is not for sale; scheduling, within these rules, is.
