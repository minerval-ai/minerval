...
#### Queue Priority and Paid Attention

Importance says how much a claim is worth getting right; it does not by itself say when the system should spend its next unit of attention. The background work queue is therefore ordered by a **composite queue priority** whose inputs are importance (the epistemic base) plus signals of *marginal value now*: the expected yield of another pass (a steward's recorded marginal-yield and contestation judgments), demand expressed as owl stakes on the claim, staleness of the current assessment, and user provenance (a human cared enough to propose it). The weights are configuration — printed, inspectable, and tunable — and the whole computation is publicly legible per claim, so "why is this claim ahead of that one" is always answerable. The composite is an input to ordering and effort selection (which model tier, what reassessment cadence), never to truth: it appears nowhere in an assessment.

Paid attention is legitimate and bounded. A user who pays for an assessment gets it *now*, on an express lane outside the queue entirely — a purchase is not a request, and it does not wait behind system work. Grant subsidies add to a claim's queue priority. But in both cases the money buys **position and coverage, never epistemic standing**: stakes and payment must never move a claim's importance, its assessment, or the standards applied to it. The same evidential rigor governs a funded assessment and an unfunded one, and the funding of an assessment is disclosed alongside it. Assessment content is not for sale; the order of the queue, within these rules, is.
